...
function getSelectedOption(sele) {
    ...
    if(!flag)
    	...
    	{
    	if(confirm("Are you sure want to update user province?"))
			   flag = true;
		 else
			flag = false;
    	}
...
<form:form modelAttribute="searchProvinceForm" action="searchUserProvince" method="post">
<div>
					<table>
						<tr>
							<td>User :<td>
				...
				<input id="save" class="wpsButtonText" type="submit" name="search"
					...
					onclick="window.location.href='<%=request.getContextPath()%>/searchProvince'"
					value="Cancel" />
			</div>
	...
	<form:form modelAttribute="searchProvinceForm" action="submitUserProvince" method="post">
	<div id="apDiv51">
			<c:if test="${not empty searchUserProvinceResults}">
		 <table id="listUserProvince" style="width: 100%" class="display">
		<tr>
			<td>Name</td>
				...
				<%-- <td><form:hidden path="urpcid" value="${searchUserProvinceResults.urpcid}"/> </td> --%>
		</tr>
		<tr>
		<td>FirstName</td>
		<td><input style="width: 175px;" id="firstName" name="firstName" value="${searchUserProvinceResults.firstName}"
					maxlength="20" type="text" readonly></td>
		</tr>
		<tr>
		<td>LastName</td>
		<td><input style="width: 175px;" id="middleName" name="middleName" value="${searchUserProvinceResults.middleName}"
					maxlength="20" type="text" readonly></td>
		</tr>
		<tr>
		<td>Branch</td>
		<td><form:select path="branchName" name="branchName" id="branchName" style="width: 175px;" multiple="true" disabled="true">
				<c:forEach items="${searchUserProvinceResults.branchName}" var="current">
		...
		<td><form:select path="roleName" name="roleName" id="roleName" style="width: 175px;" multiple="true" disabled="true">
				<c:forEach items="${searchUserProvinceResults.roleName}" var="currentRoleName">
		...
		<tr>
		<td>Product</td>
		<td><form:select path="productCategory" name="productCategory" id="productCategory" style="width: 175px;" multiple="true" disabled="true">
				<c:forEach items="${searchUserProvinceResults.productCategory}" var="currentProduct">
		...
		<tr>		
		<td>Area</td>
		<td><form:select path="areaName" name="areaName" id="areaName" style="width: 175px;" multiple="true" disabled="true">
				<c:forEach items="${searchUserProvinceResults.areaName}" var="current">
				<form:option class="option45" value="${current}" selected="selected">${current}</form:option>
				<%--<c:if test="${searchUserProvinceResults.areaName eq current.areaName}">						
						<option class="option45" value="${current.areaName}" selected>${current.areaName}</option>
					</c:if>
					 <c:if test="${searchUserProvinceResults.areaName ne current.areaName}">
					<c:out value="Seelcted"></c:out>
		...
		<td><form:select path="provinceName" name="provinceName" id="provinceName" style="width: 175px;" multiple="true">
				<c:forEach items="${searchUserProvinceResults.provinceName}" var="current">
				<form:option class="option45" value="${current}" selected="selected">${current}</form:option>
				<%--<c:if test="${searchUserProvinceResults.areaName eq current.areaName}">						
						<option class="option45" value="${current.areaName}" selected>${current.areaName}</option>
					</c:if>
		...
		<td>
		<input id="moveleft" type="button" value="<<" onclick="move_list_items('masterSetProvinceName','provinceName');" /><br><br><input id="moveright" type="button" value=">>"  onclick="move_list_items('provinceName','masterSetProvinceName');" /></td>
		
		<td><form:select path="masterSetProvinceName" name="masterSetProvinceName" id="masterSetProvinceName" style="width: 175px;" multiple="true">
		<c:forEach items="${province}" var="current">
			<form:option class="option45" value="${current.provinceName}">${current.provinceName}</form:option>
		</c:forEach>
		</form:select>
		</td>
		</tr>	
		...
		<div class="btn" style="text-align: right; dispay: block">
				<input id="save" class="wpsButtonText" type="submit" name="Submit"
					value="Submit" onclick="return getSelectedOption('provinceName')" /><input id="cancel" type="button"
					onclick="window.location.href='<%=request.getContextPath()%>/searchProvince'"
					value="Cancel" />
			</div>
		</div>
