...
<script>
	...
	function getSelectedOption(sele) {
	    ...
	    if(!flag)
	    	...
	    	{
	    		//alert('Branches selected are '+selectedVal);
	    	}
	    return flag; 
	}
...
<form:form modelAttribute="searchBranchForm" action="searchUserBranch" method="post">
<div>
					<table>
						<tr>
							<td>User :<td>
				...
				<input id="save" class="wpsButtonText" type="submit" name="search"
					...
					onclick="window.location.href='<%=request.getContextPath()%>/searchBranch'"
					value="Cancel" />
			</div>
	...
	<form:form modelAttribute="searchBranchForm" action="submitUserBranch" method="post">
	<div id="apDiv51">
			<c:if test="${not empty searchUserBranchResults}">
		 <table id="listUserBranch" style="width: 100%" class="display">
		<tr>
			<td>Name</td>
		...
		<td><form:select path="branchName" name="branchName" id="branchName" style="width: 175px;" multiple="true">
				<c:forEach items="${searchUserBranchResults.branchName}" var="current">
		...
		<td><form:select path="roleName" name="roleName" id="roleName" style="width: 175px;" multiple="true" disabled="true">
				<c:forEach items="${searchUserBranchResults.roleName}" var="currentRoleName">
		...
		<td><form:select path="productCategory" name="productCategory" id="productCategory" style="width: 175px;" multiple="true" disabled="true">
				<c:forEach items="${searchUserBranchResults.productCategory}" var="currentProduct">
		...
		<td><form:select path="branchName" name="branchName" id="branchName" style="width: 175px;" multiple="true">
				...
				<%--<c:if test="${searchUserAreaResults.areaName eq current.areaName}">						
		...
		<td>
		<input id="moveleft" type="button" value="<<" onclick="move_list_items('masterSetBranchName','branchName');" /><br><br><input id="moveright" type="button" value=">>"  onclick="move_list_items('branchName','masterSetBranchName');" /></td>
		...
		<div class="btn" style="text-align: right; dispay: block">
				<input id="save" class="wpsButtonText" type="submit" name="Submit"
					value="Submit" onclick="return getSelectedOption('branchName')"/><input id="cancel" type="button"
					onclick="window.location.href='<%=request.getContextPath()%>/searchBranch'"
					value="Cancel" />
			</div>
		</div>
